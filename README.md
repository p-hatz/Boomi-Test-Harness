...
6. Update the `(conn) AtomSphere API` (Boomi AtomSphere API) Connection
    - WSDL
    - Username
    - API Token or Password
7. Update the `(conn) AtomSphere API` (HTTP) Connection
    - Username
    - Password
8. Update the `(conn) MariaDB` Connection (or whichever DB Connection you'll be using)
    - Connection URL
    - User Name
    - Password
9. Update the `(oper) Process Test Suite (DELETE)` Operation
    - Schema
10. Update the `(oper) Process Test Suite (INSERT)` Operation
    - Schema
11. Update the `(oper) ComponentDef (INSERT)` Operation
    - Schema
### (wsvc) 1. Test Controller Process
12. Update the `(conn) MariaDB` Connection (or whichever DB Connection you'll be using)
    - Connection URL
    - User Name
    - Password
### Call the Create API Test Process
13. Provide the Id of the Process under test
>     {
...
14. Provide the Clone Id and the Id of the Process under test
>     {
